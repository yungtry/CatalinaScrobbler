...
function loop() {
    getCurrentPlaying().then(result => {
        ...
        if (songStorage.current === undefined) {
            ...
            app.emit('trayUpdate', dJSON.parse(result), false);
            if (songStorage.time >= Math.round(parseInt(dJSON.parse(songStorage.previous).duration)) / 2.8) {
            ...
            if (songStorage.time >= Math.round(parseInt(dJSON.parse(result).duration)) / 2.8 && songStorage.ready == false) {
        ...
        apple.getPlayerState().then(state => {
            console.log(state);
            if (songStorage.time >= Math.round(parseInt(dJSON.parse(songStorage.current).duration)) / 2.8 && state == "Stopped") {
