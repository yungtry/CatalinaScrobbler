...
function loop() {
    getCurrentPlaying().then(result => {
        ...
        if (songStorage.current === undefined) {
            ...
            app.emit('trayUpdate', dJSON.parse(result), false);
            if (songStorage.time >= Math.round(parseInt(dJSON.parse(songStorage.previous).duration)) / thresold) {
            ...
            if (songStorage.time >= Math.round(parseInt(dJSON.parse(result).duration)) / thresold && songStorage.ready == false) {
        ...
        apple.getPlayerState().then(state => {
            console.log(state);
            if (songStorage.time >= Math.round(parseInt(dJSON.parse(songStorage.current).duration)) / thresold && state == "Stopped") {
        ...
        if (!typeof songStorage.current === undefined) {
            apr.emit('trayUpdate', false, false);
        }
    });
}
